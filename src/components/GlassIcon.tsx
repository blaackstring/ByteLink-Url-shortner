...
const gradientMapping: Record<string, string> = {
  blue: "linear-gradient(hsl(223, 90%, 50%), hsl(208, 40%, 50%))",
  purple: "linear-gradient(hsl(223, 90%, 50%), hsl(268, 90%, 50%))",
  red: "linear-gradient(hsl(3, 90%, 50%), hsl(348, 90%, 50%))",
  indigo: "linear-gradient(hsl(253, 30%, 50%), hsl(238, 90%, 50%))",
...
const GlassIcons: React.FC<GlassIconsProps> = ({ items, className }) => {
  ...
  return (
    <div
      className={`grid gap-[3em] grid-cols-3 md:grid-cols-3 mx-auto py-4 overflow-visible ${
        className || ""
      }`}
    >
      {items.map((item, index) => (
        <a
         ...
         href={`https://${item.url}`}
          ...
          className={`relative bg-transparent outline-none  w-[2.5em] h-[2.5em] sm:w-[2.5em] sm:h-[2.5em] lg:h-[4.5em] lg:w-[4.5em] [perspective:24em] [transform-style:preserve-3d] [-webkit-tap-highlight-color:transparent] group ${
            item.customClass || ""
          }`}
        >
          <span
